...
Usually, context variables are created for storing logs (`with ... as logs`) and not used in test steps directly. For this reason, the linter ignores the context variables by default. 
...
If you need to treat this as a mistake, you can configure this in the `setup.cfg` file using the parameter `allow_unused_context_attributes=False`.
