...
For more flexibility, we left the ability to ignore specific variable names with a regex pattern. You can configure 
this in the `setup.cfg` file using the parameter `ignore_variables_pattern`:

```ini
[flake8]
ignore_variables_pattern = ^(log|history).*
```

Variables whose names match the pattern will not trigger the rule:
...
class Scenario(vedro.Scenario):
    ...
    def given_registered_user(self):
        self.log_data = registered_user(self.user)  # Not a mistake (ignored by pattern)
...
However, be careful and use this ability wisely to avoid missing real mistakes.
